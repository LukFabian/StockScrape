...
def format_to_bigint(currency: float) -> str:
...
def process_stocks(stock_data: dict, conn: sqlalchemy.Connection):
    ...
    for chart_dict in stock_data.get("chart"):
        ...
        try:
            ...
            logger.info(f"integrity error: {e}, updating instead")
            conn.rollback()
    ...
    def scrape_stocks(self, from_db=True, amount_stocks=10):
        ...
        with self.db_manager.get_connection() as conn:
            for symbol in symbols:
                ...
                time.sleep(random.randint(1, amount_stocks))


if __name__ == "__main__":
    with Scraper(os.environ["DB_URL"]) as scraper:
        scraper.scrape_stocks(from_db=True, amount_stocks=20)
